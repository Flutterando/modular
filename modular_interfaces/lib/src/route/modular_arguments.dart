...
class ModularArguments {
  /// It retrieves parameters after consulting a dynamic route. If it is not a
  /// dynamic route the object will be an empty Map.
  ...
  /// Retrieved from a direct input of arguments from the navigation system
  /// itself.
  /// ex: Modular.to.navigate('/product', arguments: Products());
  /// Modular.args.data;  -> Product();
  final dynamic data;

  ///[ModularArguments] constructor
  const ModularArguments({
    this.params = const {},
    this.data,
    required this.uri,
  });
  ///Copy the [ModularArguments] object into another memory reference
  ...
  /// post in the HTML 4.01 specification section 17.13.4.
  ...
  /// arbitrary choice of possible value. The queryParametersAll getter can 
  /// provide a map that maps keys to all of their values. The map and the 
  /// lists it contains are unmodifiable.
  ...
  ///Returns a [ModularArguments] with an [uri], pasring the string '/'
