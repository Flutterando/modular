...
void main() {
  ...
  test('assembleRoute', () {
    ...
    expect(map[const ModularKey(name: '/route')]?.name, '/route');
  });

  test('addModule', () {
    var map = routeContext
        .assembleRoute(CustomRoute(name: '/route', context: OtherModule()));
    expect(map[const ModularKey(name: '/route/')]?.uri.toString(), '/first');

    map = routeContext.assembleRoute(
        CustomRoute(name: '/route', context: OtherModuleWithlessSlash()),);
    expect(map[const ModularKey(name: '/route')]?.uri.toString(), isNull);
    expect(map[const ModularKey(name: '/route/second')]?.uri.toString(), '/second');
  });
  ...
  test('addChildren', () {
    ...
    expect(map[const ModularKey(name: '/route/2')]?.name, '/route/2');
    expect(map[const ModularKey(name: '/route/2')]?.parent, '/route');

    expect(map[const ModularKey(name: '/route/3')]?.name, '/route/3');
    expect(map[const ModularKey(name: '/route/3')]?.parent, '/route');

    expect(map[const ModularKey(name: '/route/module/')]?.name, '/route/module/');
    expect(map[const ModularKey(name: '/route/module/')]?.parent, '/route');

    expect(map[const ModularKey(name: '/route/4')]?.name, '/route/4');
    expect(map[const ModularKey(name: '/route/4')]?.parent, '/route');

    expect(map[const ModularKey(name: '/route/4/5')]?.name, '/route/4/5');
    expect(map[const ModularKey(name: '/route/4/5')]?.parent, '/route/4');

    expect(map[const ModularKey(name: '/route/4/6')]?.name, '/route/4/6');
    expect(map[const ModularKey(name: '/route/4/6')]?.parent, '/route/4');
  });

  test('order route', () {
    final list = [
      const ModularKey(name: '/event/n/**'),
      const ModularKey(name: '/:id'),
      const ModularKey(name: '/event/**'),
      const ModularKey(name: '/**'),
      const ModularKey(name: '/route/:id'),
      const ModularKey(name: '/route/other'),
      const ModularKey(name: '/route/id'),
    ];
